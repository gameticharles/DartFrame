  ...
  group('DataFrame.getDummies()', () {
    ...
    group('Basic Functionality:', () {
      test('Single column specified', () {
        ...
        expect(result.columns,
            containsAll(['Category_A', 'Category_B', 'Category_C']));
      ...
      test('Multiple columns specified', () {
        ...
        expect(result.columns, containsAll(['ID', 'Value']));
        expect(result.columns,
            containsAll(['Cat1_A', 'Cat1_B', 'Cat2_X', 'Cat2_Y']));
    ...
    group('Column Auto-Detection (columns = null):', () {
      test('Auto-detects string columns, leaves others', () {
        ...
        expect(result.columns,
            containsAll(['ID', 'ColInt', 'ColBool', 'ColDouble']));
    ...
    group('prefix Parameter:', () {
      ...
      test('Single string prefix for multiple columns (applies to all)', () {
        ...
        expect(
            result.columns, containsAll(['PFX_A', 'PFX_B', 'PFX_X', 'PFX_Y']));
    ...
    group('prefixSep Parameter:', () {
      test('Custom prefixSep', () {
        ...
        expect(result.columns,
            containsAll(['Category|A', 'Category|B', 'Category|C']));
      });

      test('Custom prefixSep with prefix', () {
        final result =
            dfSimple.getDummies(['Category'], prefix: 'Type', prefixSep: '--');
    ...
    group('dummyNA Parameter:', () {
      test('dummyNA = true creates NA column', () {
        ...
        expect(result.column('Category_A').data,
            equals([1, 0, 1, 0, 0, 0])); // Ensure other columns are correct
      });
    ...
    group('dropFirst Parameter:', () {
      test('dropFirst = true removes first category', () {
        ...
        final result =
            dfSimple.getDummies(['Category'], dropFirst: true, dummyNA: false);
        expect(result.columns, isNot(contains('Category_A')));
      ...
      test('dropFirst = true with dummyNA = true', () {
        ...
        // Drops A.
        final result =
            dfSimple.getDummies(['Category'], dropFirst: true, dummyNA: true);
        expect(result.columns, isNot(contains('Category_A')));
        expect(result.columns,
            containsAll(['Category_B', 'Category_C', 'Category_na']));
        expect(result.column('Category_na').data, equals([0, 0, 0, 0, 0, 1]));
      });

      test('dropFirst = false (default)', () {
        final result =
            dfSimple.getDummies(['Category'], dropFirst: false, dummyNA: false);
        expect(result.columns,
            containsAll(['Category_A', 'Category_B', 'Category_C']));
      });
    });
    ...
    group('Column Naming and Uniqueness:', () {
      test('Generated name conflicts with existing column', () {
        ...
        expect(result.columns,
            contains('Category_B_1')); // Assuming _1 suffix for conflict
        expect(result.columns,
            contains('Category_B')); // Original conflicting column
      });

      test('Category values are numbers', () {
        final result = dfWithNumericCat.getDummies(['NumCat']);
        expect(result.columns,
            containsAll(['ID', 'Data', 'NumCat_101', 'NumCat_102']));
      ...
      test('Category values with special characters (default prefixSep)', () {
        final dfSpecial = DataFrame.fromRows([
          {'Spec': 'val*1'},
          {'Spec': 'val?2'}
        ]);
    ...
    group('Edge Cases:', () {
      test('DataFrame with no columns to encode (all numeric)', () {
        final dfNum = DataFrame.fromRows([
          {'X': 1, 'Y': 2}
        ]);
      ...
      test('Column with only one unique value', () {
        final dfOneUnique = DataFrame.fromRows([
          {'Cat': 'A'},
          {'Cat': 'A'}
        ]);
        ...
        final resultDropFirst =
            dfOneUnique.getDummies(['Cat'], dropFirst: true);
        expect(resultDropFirst.columnCount,
            equals(0)); // The only category 'A' is dropped
      });
      ...
      test('Column with only missing values', () {
        ...
        expect(resultNoDummyNa.columnCount,
            equals(0)); // No categories, no NA column
        ...
        final resultDummyNaDropFirst =
            dfOnlyNa.getDummies(['Col'], dummyNA: true, dropFirst: true);
