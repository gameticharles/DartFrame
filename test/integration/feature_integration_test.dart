...
void main() {
  ...
  group('I/O Integration', () {
    ...
    test('Database integration works with existing DataFrame features', () async {
  ...
  group('Performance Integration', () {
    ...
    test('Vectorized operations integrate with existing Series methods', () async {
      final series = Series(List.generate(100, (i) => i.toDouble()), name: 'test_series');
  ...
  group('Error Handling Integration', () {
    test('Enhanced error handling maintains existing behavior', () {
      ...
      expect(() => df.corrAdvanced(method: 'invalid'), throwsA(isA<ArgumentError>()));
      
      // Test that operations on empty DataFrame handle errors
      final emptyDf = DataFrame.empty();
      expect(() => emptyDf.std(), returnsNormally); // Empty DataFrame should handle gracefully
    });
  ...
  group('Backward Compatibility', () {
    test('All existing DataFrame constructors still work', () {
      ...
      final fromMap = DataFrame.fromMap({'A': [1, 2, 3]});
    ...
    test('Existing indexing and slicing operations work', () {
      ...
      final boolSeries = Series([true, false, true, false, true], name: 'filter');
