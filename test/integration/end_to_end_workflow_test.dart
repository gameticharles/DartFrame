...
void main() {
  group('Data Analysis Workflow', () {
    test('Complete data analysis pipeline', () async {
      ...
      final rawData = {
        'date': List.generate(
            size, (i) => DateTime(2023, 1, 1).add(Duration(days: i ~/ 4))),
        'category': List.generate(size, (i) => ['A', 'B', 'C'][i % 3]),
        'value': List.generate(
            size, (i) => random.nextDouble() * 100 + (i % 3) * 10),
        'count': List.generate(size, (i) => random.nextInt(50) + 10),
        'flag': List.generate(size, (i) => random.nextBool()),
      };
    ...
    test('Performance workflow with large dataset', () async {
      ...
      final largeData = {
        ...
        'category':
            List.generate(size, (i) => ['A', 'B', 'C', 'D', 'E'][i % 5]),
      };
