...
void main() {
  ...
  group('DataFrame.join() with "on" parameter:', () {
    test('inner join with "on" as String', () {
      ...
      expect(
          result.rows.map((r) => Map.fromIterables(result.columns, r)).toList(),
          containsAll([
    ...
    test('right join with "on" as String', () {
      final result =
          df1.join(df2, on: 'key', how: 'right', suffixes: ['_l', '_r']);
      expect(result.rowCount, equals(4));
      expect(result.columns, equals(['key', 'A', 'B', 'C', 'D']));
      final resultRowsAsMaps =
          result.rows.map((r) => Map.fromIterables(result.columns, r)).toList();
      expect(
          resultRowsAsMaps,
          containsAllInOrder([
    ...
    test('outer join with "on" as String', () {
      final result =
          df1.join(df2, on: 'key', how: 'outer', suffixes: ['_l', '_r']);
    ...
    test('Error when "on" is used with "leftOn"', () {
      expect(() => df1.join(df2, on: 'key', leftOn: 'key', how: 'inner'),
          throwsArgumentError);
    });

    test('Error when "on" is used with "rightOn"', () {
      expect(() => df1.join(df2, on: 'key', rightOn: 'key', how: 'inner'),
          throwsArgumentError);
    });

    test('Error when "on" column does not exist in left DataFrame', () {
      expect(() => df1.join(df2, on: 'wrong_key', how: 'inner'),
          throwsArgumentError);
    });

    test('Error when "on" column does not exist in right DataFrame', () {
      final dfTemp = DataFrame.fromRows([
        {'wrong_key': 'K0'}
      ]);
      expect(
          () => df1.join(dfTemp,
              on: 'key', how: 'inner'), // 'key' is not in dfTemp
          throwsArgumentError);
    });
  });
  ...
  group('DataFrame.join() with "indicator" parameter:', () {
    ...
    test('indicator="custom_name" for left join', () {
      final result =
          df1.join(df2, on: 'key', how: 'left', indicator: 'source_info');
    ...
    test('indicator column name conflict (default _merge)', () {
      ...
      df1WithMerge.addColumn('_merge',
          defaultValue: List.filled(df1.rowCount, 'original'));
      final result =
          df1WithMerge.join(df2, on: 'key', how: 'left', indicator: true);
      ...
      expect(
          result.column('_merge').data.every((v) => v == 'original'), isTrue);
      expect(result.column('_merge_1').data,
          equals(['both', 'both', 'both', 'left_only']));
    });

    test('indicator column name conflict (custom name)', () {
      final df1WithCustom = df1.copy();
      df1WithCustom.addColumn('custom_indicator',
          defaultValue: List.filled(df1.rowCount, 'original_custom'));
      final result = df1WithCustom.join(df2,
          on: 'key', how: 'left', indicator: 'custom_indicator');
      expect(result.columns, contains('custom_indicator_1'));
      expect(result.columns, contains('custom_indicator'));
      expect(
          result
              .column('custom_indicator')
              .data
              .every((v) => v == 'original_custom'),
          isTrue);
      expect(result.column('custom_indicator_1').data,
          equals(['both', 'both', 'both', 'left_only']));
    });
  });
  ...
  group('DataFrame.join() - Combinations and Edge Cases:', () {
    test('left join with "on" and indicator=true, different lengths', () {
      ...
      final result =
          dfShort.join(dfLong, on: 'id', how: 'left', indicator: true);
      expect(result.rowCount, equals(2));
    ...
    test('outer join with "on" and indicator="source", non-overlapping keys',
      ...
      final result =
          dfLeft.join(dfRight, on: 'key', how: 'outer', indicator: 'source');
      expect(result.rowCount, equals(2));
    ...
    test('join with missing values in join keys (inner)', () {
      ...
      final dfB = DataFrame.fromRows([
        {'key': 'K0', 'valB': 'B0'},
        {
          'key': null,
          'valB': 'B1'
        }, // Will not match null in dfA due to null != null
        {'key': 'K2', 'valB': 'B2'},
      ]);
  ...
  group('DataFrame.join() - Regression Tests for leftOn/rightOn and suffixes:',
    ...
    test('inner join with leftOn/rightOn and suffixes', () {
      ...
      final result = dfLeft.join(dfRight,
          leftOn: 'lkey',
          rightOn: 'rkey',
          how: 'inner',
          suffixes: ['_L', '_R']);
    ...
    test('left join with conflicting column names and suffixes', () {
      ...
      final result =
          dfX.join(dfY, on: 'id', how: 'left', suffixes: ['_dfX', '_dfY']);
  ...
  group('DataFrame.concatenate()', () {
    ...
    group('axis = 0 (row-wise)', () {
      ...
      test('Concatenate with an empty DataFrame (cols) - outer join', () {
        ...
        expect(
            result.rowCount, equals(4)); // 2 from df_a1, 2 from df_empty_cols
        ...
        expect(
            ...
            equals([
              ...
              [
                null,
                null
              ] // df_empty_cols contributes nulls for df_a1's columns
            ]));
      });
      ...
      test('join="inner" with no common columns', () {
        final dfC1 = DataFrame.fromRows([
          {'X': 1}
        ]);
        final dfC2 = DataFrame.fromRows([
          {'Y': 2}
        ]);
    ...
    group('axis = 1 (column-wise)', () {
      final dfR1 = DataFrame.fromRows([
        {'A': 1, 'B': 2},
        {'A': 3, 'B': 4}
      ]);
      final dfR2 = DataFrame.fromRows([
        {'C': 5, 'D': 6},
        {'C': 7, 'D': 8}
      ]);
      final dfR3Short = DataFrame.fromRows([
        {'E': 9}
      ]); // Fewer rows
      final dfR4Long = DataFrame.fromRows([
        {'F': 10},
        {'F': 11},
        {'F': 12}
      ]); // More rows
      final dfR5DupCol = DataFrame.fromRows([
        {'A': 10, 'G': 20},
        {'A': 30, 'G': 40}
      ]);
      ...
      test('axis=1 with ignore_index=true', () {
        ...
        expect(result.columns,
            equals([0, 1, 2, 3])); // Default integer column names
        expect(
            result.rows,
            equals([
              [1, 2, 5, 6],
      ...
      test('axis=1 with duplicate column names, ignore_index=false', () {
        final result =
            dfR1.concatenate([dfR5DupCol], axis: 1, ignoreIndex: false);
    ...
    group('Edge Case Concatenations', () {
      ...
      test(
          'Concatenate completely empty DataFrame with another (axis=0, outer)',
          () {
      ...
      test(
          'Concatenate completely empty DataFrame with another (axis=1, outer)',
          () {
        final result = dfEmptyAll.concatenate([dfA1], axis: 1, join: 'outer');
        expect(result.rowCount, equals(2)); // max rows
        expect(result.columns,
            equals(['col1', 'col2'])); // df_empty_all has 0 cols
        expect(
            result.rows,
            equals([
              [1, 'a'],
              [
                2,
                'b'
              ] // df_a1 data, df_empty_all contributes nothing to rows/cols
            ]));
      });
      ...
      test('Concatenate DataFrame with empty (cols) DataFrame (axis=1, outer)',
        ...
        expect(result.columns,
            equals(['col1', 'col2'])); // df_empty_cols has 0 named cols
        expect(
            result.rows,
            equals([
              [1, 'a'],
