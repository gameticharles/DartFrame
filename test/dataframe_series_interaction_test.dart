...
void main() {
  ...
  group('Series from DataFrame Column', () {
    ...
    test('Series has parent DataFrame reference and column name', () {
      ...
      expect(sA, isNotNull); // Placeholder for actual check if public accessors were available
    });
    ...
    test('isna()/notna() use DataFrame replaceMissingValueWith', () {
      ...
      Series sNull = Series([1, null, 3], name: 'sNull'); // No parent DF, null is missing
  ...
  group('Setting DataFrame Column with a Series', () {
    test('Set with Series, identical index', () {
      ...
      expect(df['B'].index, equals(['x', 'y', 'z'])); // Series index becomes DF column index
    });

    test('Set with Series, different but aligned index (subset of DF index)', () {
      var df = DataFrame.fromMap({'A': [1, 2, 3, 4]}, index: ['w', 'x', 'y', 'z']);
      final s = Series([10, 20], name: 'B', index: ['x', 'y']); // Index is subset of df
      df['B'] = s; // DataFrame's addColumn or equivalent logic handles alignment

      expect(df.columns, equals(['A', 'B']));
      expect(df['B'].data, equals([null, 10, 20, null])); // Aligns, others are null (or df.replaceMissingValueWith)
      expect(df['B'].index, equals(['w', 'x', 'y', 'z']));
    });
    
    test('Set with Series, different but aligned index (superset of DF index)', () {
      var df = DataFrame.fromMap({'A': [1, 2]}, index: ['x', 'y']);
      final s = Series([10, 20, 30, 40], name: 'B', index: ['w', 'x', 'y', 'z']);
      df['B'] = s;
    ...
    test('Set with Series, default integer index (assign by row order)', () {
      var df = DataFrame.fromMap({'A': [1, 2, 3]}, index: ['x', 'y', 'z']);
      final s = Series([10, 20, 30], name: 'B'); // Default index [0, 1, 2]
      df['B'] = s;
    ...
    test('Set with Series, length mismatch (shorter Series, default index)', () {
      ...
      expect(df['B'].data, equals([10, 20, null, null])); // Or df.replaceMissingValueWith
    });
  ...
  group('Modifying a Series obtained from a DataFrame', () {
    test('Modification of extracted Series updates DataFrame', () {
      ...
      expect(df['colA'].data[0], equals(100), reason: "DataFrame should reflect Series modification via s[idx]=val");
