  ...
  group('Series.to_datetime()', () {
    ...
    group('Basic String Conversions:', () {
      test('ISO 8601 strings (yyyy-MM-ddTHH:mm:ss)', () {
        final s = Series(['2023-10-26T10:30:00', '2024-01-15T23:15:45'],
            name: 'iso_datetime');
        final result = s.toDatetime();
        expect(
            result.data,
            equals([
              DateTime(2023, 10, 26, 10, 30, 0),
              DateTime(2024, 1, 15, 23, 15, 45)
            ]));
      ...
      test('ISO 8601 strings (yyyy-MM-dd, time defaults to midnight)', () {
        ...
        final result = s.toDatetime();
        expect(result.data,
            equals([DateTime(2023, 10, 26), DateTime(2024, 1, 15)]));
      });

      test('ISO 8601 strings with Z (UTC)', () {
        final s = Series(['2023-10-26T10:30:00Z', '2024-01-15T00:00:00Z'],
            name: 'iso_utc');
        final result = s.toDatetime();
        expect(
            result.data,
      ...
      test(
          'String with only time part (unsupported by default, handled by errors)',
          () {
        final s = Series(['10:30:00'], name: 'time_only');
        expect(() => s.toDatetime(errors: 'raise'),
    ...
    group('`format` Parameter:', () {
      test('Correct format "yyyy/MM/dd HH:mm"', () {
        final s = Series(['2023/10/26 10:45', '2024/01/15 05:15'],
            name: 'custom_format');
        final result = s.toDatetime(format: 'yyyy/MM/dd HH:mm');
        expect(
            result.data,
            equals([
              DateTime(2023, 10, 26, 10, 45),
              DateTime(2024, 1, 15, 5, 15)
            ]));
      });

      test('Non-matching format string with errors="raise"', () {
        final s = Series(['2023-10-26'], name: 'wrong_format');
        expect(() => s.toDatetime(format: 'yyyy/MM/dd', errors: 'raise'),
            throwsA(isA<FormatException>()));
      });
    ...
    group('`infer_datetime_format` true vs. false:', () {
      test('Infer succeeds where default tryParse fails', () {
        ...
        expect(() => s2.toDatetime(inferDatetimeFormat: false, errors: 'raise'),
            throwsA(isA<FormatException>()));
      });
    });
    ...
    group('Numeric Timestamps (Milliseconds Since Epoch):', () {
      test('Integer timestamps', () {
        ...
        expect(
            result.data,
            equals([
              DateTime.fromMillisecondsSinceEpoch(ts1),
              DateTime.fromMillisecondsSinceEpoch(ts2)
            ]));
      });

      test('Double timestamps (converted to int)', () {
        final ts1Double =
            DateTime(2023, 5, 10, 8, 0, 0).millisecondsSinceEpoch.toDouble();
        final ts1Int = ts1Double.toInt();
        final s = Series([ts1Double, 1678886400000.0],
            name: 'double_ts'); // 2023-03-15T12:00:00.000Z
        final result = s.toDatetime();
        expect(
            result.data,
            equals([
    ...
    group('`errors` Parameter Behavior:', () {
      final sInvalid = Series(['2023-13-01', 'not-a-date', 12345, '2023-01-01'],
          name: 'invalid_dates');
      ...
      test("errors = 'raise' (default)", () {
        ...
        expect(sValid.toDatetime().data.first,
            equals(DateTime(2022, 1, 1))); // Ensure valid still works
      });
      ...
      test('Invalid "errors" value throws ArgumentError', () {
        expect(() => sInvalid.toDatetime(errors: 'unknown_policy'),
            throwsArgumentError);
      });
    });
    ...
    group('Handling of Missing Values in Input:', () {
      test('Series with nulls (default missing)', () {
        ...
        final result = s.toDatetime(
            errors: 'coerce'); // errors shouldn't affect already null values
    ...
    group('Result Properties:', () {
      test('Name and Index are preserved', () {
        ...
        final s = Series(['2023-01-01', '2023-01-02'],
            name: 'test_name_dt', index: originalIndex);
        final result = s.toDatetime();
  ...
  group('dateRange() top-level function', () {
    group('Parameter Combinations:', () {
      ...
      test('start and end specified', () {
        final result =
            dateRange(start: DateTime(2023, 1, 1), end: DateTime(2023, 1, 3));
        expect(
            result.data,
            equals([
              DateTime(2023, 1, 1),
      ...
      test('Error: only start specified', () {
        expect(
            () => dateRange(start: DateTime(2023, 1, 1)), throwsArgumentError);
      });
      ...
      test('All three specified (consistent)', () {
        final result = dateRange(
            start: DateTime(2023, 1, 1), end: DateTime(2023, 1, 3), periods: 3);
        expect(
            result.data,
            equals([
              DateTime(2023, 1, 1),
    ...
    group('freq="D" (Daily Frequency):', () {
      ...
      test('Verify daily increment with start and end crossing month', () {
        final result =
            dateRange(start: DateTime(2023, 2, 27), end: DateTime(2023, 3, 2));
        expect(
            result.data,
            equals([
              DateTime(2023, 2, 27),
    ...
    group('normalize Parameter:', () {
      test('normalize = true with start and end', () {
        final result = dateRange(
            ...
            normalize: true);
        expect(
            result.data, equals([DateTime(2023, 1, 1), DateTime(2023, 1, 2)]));
      });
      test('normalize = true with start and periods', () {
        final result = dateRange(
            start: DateTime(2023, 1, 1, 10, 30), periods: 2, normalize: true);
        expect(
            result.data, equals([DateTime(2023, 1, 1), DateTime(2023, 1, 2)]));
      });
      test(
          'normalize = false (default) preserves time in first element if start has time',
          () {
        final startDt = DateTime(2023, 1, 1, 10, 30);
        final result = dateRange(start: startDt, periods: 2, normalize: false);
        expect(result.data,
            equals([startDt, startDt.add(const Duration(days: 1))]));
      });
    });
    ...
    group('name Parameter:', () {
      test('Custom name provided', () {
        final result = dateRange(
            start: DateTime(2023, 1, 1), periods: 1, name: 'MyDateRange');
    ...
    group('periods Parameter Edge Cases:', () {
      ...
      test('periods = 0 with start and end (consistent)', () {
        final result = dateRange(
            ...
            periods: 0); // start after end, but periods=0
        expect(result.data, isEmpty);
      });
      ...
      test('periods < 0 throws ArgumentError', () {
        expect(() => dateRange(start: DateTime(2023, 1, 1), periods: -1),
            throwsArgumentError);
      });
    });
    ...
    group('start and end Edge Cases:', () {
      test('start after end, no periods (empty or error)', () {
        ...
        final result =
            dateRange(start: DateTime(2023, 1, 5), end: DateTime(2023, 1, 1));
        expect(result.data, isEmpty);
      });
      ...
      test('start equal to end, no periods', () {
        final result =
            dateRange(start: DateTime(2023, 1, 1), end: DateTime(2023, 1, 1));
        expect(result.data, equals([DateTime(2023, 1, 1)]));
      });
      test('start equal to end, periods = 1', () {
        final result = dateRange(
            start: DateTime(2023, 1, 1), end: DateTime(2023, 1, 1), periods: 1);
        expect(result.data, equals([DateTime(2023, 1, 1)]));
      });
      test('start equal to end, periods = 0', () {
        final result = dateRange(
            start: DateTime(2023, 1, 1), end: DateTime(2023, 1, 1), periods: 0);
        expect(result.data, isEmpty);
      });
    });
    ...
    group('Result Properties:', () {
      test('Output is a Series', () {
        expect(
            dateRange(start: DateTime(2023, 1, 1), periods: 1), isA<Series>());
      });
    ...
    group('Unsupported freq:', () {
      test('freq = "H" throws ArgumentError', () {
        expect(
            () => dateRange(start: DateTime(2023, 1, 1), periods: 1, freq: 'H'),
            throwsArgumentError);
      });
    });
  });
