...
void main() {
  group('Series.str Accessor', () {
    ...
    var sBase = Series([' Hello', 'World ', ' DartFrame ', null, '  '],
        name: 'strings', index: ['a', 'b', 'c', 'd', 'e']);
    ...
    var dfSpecificMissing =
        DataFrame.empty(replaceMissingValueWith: 'MISSING_VAL');
    var sMixedType = Series([' One ', 'MISSING_VAL', 'Three', 42, null],
        name: 'mixed', index: ['x', 'y', 'z', 'w', 'v']);
    ...
    group('str.lower()', () {
      test('on string series', () {
        ...
        expect(
            result.data,
            equals(
                [' hello', 'world ', ' dartframe ', defaultMissingRep, '  ']));
    ...
    group('str.strip()', () {
      test('on string series', () {
        ...
        expect(result.data,
            equals(['Hello', 'World', 'DartFrame', defaultMissingRep, '']));
      });
      test('on mixed type series', () {
    ...
    group('str.startswith()', () {
      test('on string series', () {
        ...
        expect(result.data,
            equals([true, false, false, defaultMissingRep, false]));
    ...
    group('str.endswith()', () {
      test('on string series', () {
        ...
        expect(
            result.data, equals([false, true, true, defaultMissingRep, true]));
      });
      test('on mixed type series', () {
    ...
    group('str.contains()', () {
      test('on string series', () {
        ...
        expect(result.data,
            equals([false, true, false, defaultMissingRep, false]));
      });
      test('on mixed type series', () {
    ...
    group('str.replace()', () {
      test('on string series with String pattern', () {
        ...
        expect(
            result.data,
            equals(
                ['_Hello', 'World_', '_DartFrame_', defaultMissingRep, '__']));
        expect(result.name,
            contains('strings_replace_')); // Name can be complex due to pattern
      });

      test('on string series with RegExp pattern', () {
        var result = sBase.str
            .replace(RegExp(r'\s+'), '_'); // Replace all whitespace blocks
        expect(
            result.data,
            equals(
                ['_Hello', 'World_', '_DartFrame_', defaultMissingRep, '_']));
      });
    ...
    group('str.match()', () {
      test('match basic regex', () {
        var s = Series(['apple1', 'banana2', 'orange3', null, 'grape4'],
            name: 'match_series');
      ...
      test('match no match returns missing', () {
        ...
        expect(result.data,
            equals([defaultMissingRep, defaultMissingRep, defaultMissingRep]));
      });
      ...
      test('match extracts first group if regex has groups', () {
        var s =
            Series(['item_123', 'product_45', 'detail_6'], name: 'group_match');
        var result = s.str.match(
            RegExp(r'item_(\d+)')); // Capture group for digits after 'item_'
        expect(
            result.data, equals(['123', defaultMissingRep, defaultMissingRep]));
      });
    });
  });
}
