  ...
  group('Series Indexing and Accessing', () {
    ...
    group('Series.operator [] (get)', () {
      test('get by integer position', () {
        ...
        expect(() => s[3], throwsRangeError); // Or specific IndexError if implemented
    ...
    group('Series.operator []= (set)', () {
      ...
      test('set by boolean list with list of values', () {
        ...
        expect(() => s[[true, false, true, false]] = [100,300] , throwsArgumentError); // Mismatched lengths
        ...
        s2[[true, false, true, false]] = [99, 0, 88, 0]; // Value list matches mask length
        expect(s2.data, equals([99, 2, 88, 4])); // Only 99 and 88 applied
      });

      test('set with parent DataFrame updates parent', () {
        var df = DataFrame.fromMap({'colA': [1, 2, 3], 'colB': [4, 5, 6]});
        Series s = df['colA'];
