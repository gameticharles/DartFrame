...
void main() {
  ...
  group('Series.toNumeric()', () {
    test('toNumeric basic string to int/double', () {
      ...
      expect(numericS.data.map((e) => e.runtimeType).toList(), equals([int, double, int, double]));
    });
    ...
    test('toNumeric errors="raise" (default)', () {
      ...
      expect(() => s.toNumeric(), throwsA(isA<FormatException>()));
    });
    ...
    test('toNumeric errors="coerce" with custom missing marker', () {
      var df = DataFrame.fromMap({'col': ['1', 'abc', '3.0', 'xyz']}, replaceMissingValueWith: missingMarker);
      Series s = df['col'];
      s.setParent(df, 'col');
      final numericS = s.toNumeric(errors: 'coerce');
    ...
    test('toNumeric errors="ignore"', () {
      ...
      expect(numericS.data, equals([1, 'abc', 3.0, true])); // 'abc' and true remain
    });
    ...
    test('toNumeric downcast="integer"', () {
      ...
      expect(numericS.data.map((e) => e.runtimeType).toList(), equals([int, double, int, int]));
    });
    ...
    test('toNumeric with existing missing values (custom marker)', () {
       var df = DataFrame.fromMap({'col': ['1', missingMarker, '2.5']}, replaceMissingValueWith: missingMarker);
       Series s = df['col'];
       s.setParent(df, 'col');
       final numericS = s.toNumeric(errors: 'coerce');
  ...
  group('Series.toDatetime()', () {
    ...
    test('toDatetime basic ISO 8601 strings', () {
      final s = Series(['2023-10-26', '2024-03-01T14:30:00', null], name: 'iso_dates');
    ...
    test('toDatetime errors="coerce" with null as missing', () {
      final s = Series(['2023-10-26', 'invalid', null], name: 'coerce_date_nulls');
    ...
    test('toDatetime errors="coerce" with custom missing marker', () {
      var df = DataFrame.fromMap({'col': ['2023-10-26', 'invalid', 'xyz']}, replaceMissingValueWith: missingMarker);
      Series s = df['col'];
      s.setParent(df, 'col');
      final datetimeS = s.toDatetime(errors: 'coerce');
    ...
    test('toDatetime errors="ignore"', () {
      final s = Series(['2023-10-26', 'invalid', true], name: 'ignore_date_errors');
    ...
    test('toDatetime with specific format', () {
      ...
      final sDateOnly = Series(['26/10/2023', '01/03/2024'], name: 'custom_date_only');
      final dtDateOnly = sDateOnly.toDatetime(format: 'dd/MM/yyyy');
      expect(dtDateOnly.data, equals([DateTime(2023,10,26), DateTime(2024,3,1)]));

      final sDateTime = Series(['26/10/2023 10:00', '01/03/2024 14:30'], name: 'custom_datetime');
      final dtDateTime = sDateTime.toDatetime(format: 'dd/MM/yyyy HH:mm');
      expect(dtDateTime.data, equals([DateTime(2023,10,26,10,0), DateTime(2024,3,1,14,30)]));
    });

    test('toDatetime inferDatetimeFormat=true', () {
      final s = Series(['10/26/2023', '2024-03-01 14:30:00', '01.Mar.2024'], name: 'infer_dates');
      ...
      final datetimeS = s.toDatetime(inferDatetimeFormat: true, errors: 'coerce');
