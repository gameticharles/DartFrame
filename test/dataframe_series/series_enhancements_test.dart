...
void main() {
  ...
  group('Series.sortValues() tests', () {
    ...
    test('sorts with missing values (custom marker) and naPosition="first"',
      ...
      var df = DataFrame.fromMap({
        'col1': [10, missingMarker, 5, 20, missingMarker]
      }, replaceMissingValueWith: missingMarker);
      Series s = df['col1'];
      s.setParent(df,
          'col1'); // Manually set parent for testing replaceMissingValueWith
    ...
    test(
        'sorts series with non-directly comparable types (maintains order for uncomparables)',
        () {
      final s = Series([10, 'apple', 5, 'banana'],
          name: 'mixed_types', index: ['a', 'b', 'c', 'd']);

      final sortedS = s.sortValues();
      expect(sortedS.data,
          equals([5, 10, 'apple', 'banana'])); // Based on current fallback
  ...
  group('Series.sortIndex() tests', () {
    ...
    test('sorts with duplicate index values', () {
      ...
      final s = Series([10, 20, 30, 40, 50],
          name: 'dupe_idx', index: ['b', 'a', 'c', 'a', 'b']);
      final sortedS = s.sortIndex();
    ...
    test('sorts index with null values (nulls first)', () {
      final s = Series([10, 20, 30, 40],
          name: 'idx_with_nulls', index: [1, null, 0, null]);
      final sortedS = s
          .sortIndex(); // Default null handling should place them first or as per Comparable
      ...
      expect(
          ...
          equals([
            ...
            30,
            10
          ])); // data corresponding to sorted null, null, 0, 1
  ...
  group('Series.resetIndex() tests', () {
    ...
    test('drop=false, returns DataFrame, named Series, named index', () {
      ...
      expect(result.index,
          equals([0, 1, 2])); // DataFrame gets its own default index
    });
    ...
    test('drop=false, unnamed Series (name=\'\'), default index column name',
      ...
      final result =
          s.resetIndex(drop: false); // name for index col defaults to 'index'

      expect(result, isA<DataFrame>());
      expect(result.columns,
          equals(['index', '0'])); // Unnamed series data col becomes '0'
      expect(result.column('index').data, equals(['a', 'b']));
      expect(result.column('0').data, equals([10, 20]));
    });

    test(
        'drop=false, Series name is "0", index column name defaults to "index"',
        () {
    ...
    test(
        'drop=false, Series name is "index", index column name also "index" (conflict)',
        () {
      ...
      final result =
          s.resetIndex(drop: false); // Index col name defaults to 'index'
    ...
    test(
        'drop=false, custom name for index column that conflicts with Series name',
        () {
    ...
    test('empty series, drop=false', () {
      ...
      expect(result, isA<DataFrame>());
      expect(
          result.columns, equals(['idx', 'empty'])); // or '0' if name was empty
  ...
  group('Series.fillna() with method tests', () {
    ...
    test('fillna with custom missing marker (bfill)', () {
      var df = DataFrame.fromMap({
        'col1': [missingMarker, 10, missingMarker, 20, missingMarker]
      }, replaceMissingValueWith: missingMarker);
      Series s = df['col1'];
      s.setParent(df, 'col1');
    ...
    test('fillna with value takes precedence if method is invalid', () {
      ...
      expect(() => s.fillna(value: 99, method: 'invalid_method'),
          throwsArgumentError);
  ...
  group('Series.dt accessor tests', () {
    final dt1 = DateTime(2023, 10, 26, 14, 30, 15, 500,
        250); // Year, Month, Day, Hour, Min, Sec, Millis, Micros
    ...
    test('dt.weekday', () {
      // Monday=1 ... Sunday=7
      final s = Series([dt1, dt2, null],
          name:
              'dates'); // 2023-10-26 is Thursday (4), 2024-03-01 is Friday (5)
    ...
    test('dt properties with non-DateTime and missing values', () {
      ...
      expect(yearS.data,
          equals([2023, missingMarker, missingMarker, missingMarker, 2024]));

      final monthS = s.dt.month;
      expect(monthS.data,
          equals([10, missingMarker, missingMarker, missingMarker, 3]));
    });
  ...
  group('Series.apply() tests', () {
    ...
    test('apply with missing values (custom marker)', () {
      var df = DataFrame.fromMap({
        'col1': [1, missingMarker, 3]
      }, replaceMissingValueWith: missingMarker);
      Series s = df['col1'];
      s.setParent(df, 'col1');

      final result =
          s.apply((x) => x == missingMarker ? 'is_missing' : x + 100);
  ...
  group('Series.unique() tests', () {
    ...
    test('unique with mixed types', () {
      final s = Series([1, 'a', 1, null, 'a', true, null, true, 2.0],
          name: 'mixed_unique');
