  ...
  group('Rolling Methods Compatibility Tests', () {
    ...
    group('New rollingWindow() Method', () {
      ...
      test('rollingWindow() produces expected results', () {
        ...
        expect(result.iloc(2, 0), equals(4.0));  // Third value: mean of [1, 4, 7]
        expect(result.iloc(3, 0), equals(7.0));  // Fourth value: mean of [4, 7, 10]
        expect(result.iloc(4, 0), equals(10.0)); // Fifth value: mean of [7, 10, 13]
      ...
      test('both methods produce same results for mean operation', () {
        ...
        for (int i = 0; i < deprecatedResult.length; i++) {
          if (deprecatedResult.data[i] == null) {
            ...
            expect(newColumnA.data[i], closeTo(deprecatedResult.data[i], 0.001));
          }
        }
      });
      ...
      test('both methods produce same results for std operation', () {
        ...
        for (int i = 0; i < deprecatedResult.length; i++) {
          if (deprecatedResult.data[i] == null) {
            ...
            expect(newColumnC.data[i], closeTo(deprecatedResult.data[i], 0.001));
          }
        }
      });
    });
    ...
    group('Migration Examples', () {
      ...
      test('new method provides additional functionality not available in deprecated method', () {
        var rolling = df.rollingWindow(3);
        ...
        // Custom function
        expect(() => rolling.apply((window) => window.length.toDouble()), returnsNormally);
