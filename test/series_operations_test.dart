...
void main() {
  ...
  group('Series Arithmetic & Bitwise Operations with Index Alignment', () {
    ...
    var s3 = Series([1, 2, 300],
        name: 's3', index: ['a', 'b', 'e']); // Overlaps 'a', 'b' with s1/s2
    var s4 = Series([10, 400, 50],
        name: 's4', index: ['b', 'f', 'g']); // Overlaps 'b' with s1/s2/s3

    // Completely different indexes
    var s5 = Series([100, 200], name: 's5', index: ['x', 'y']);

    // Series with missing values (null as placeholder)
    var sMiss1 = Series([1, null, 3, null],
        name: 's_miss_1', index: ['a', 'b', 'c', 'd']);
    var sMiss2 = Series([null, 10, null, 20],
        name: 's_miss_2', index: ['a', 'b', 'c', 'd']);
    ...
    var sSpecMiss1 = Series([-999, 1, 2, -999],
        name: 's_spec_miss_1', index: ['a', 'b', 'c', 'd']);
    sSpecMiss1.setParent(dfSpecificMissing, 's_spec_miss_1');
    var sSpecMiss2 = Series([10, -999, -999, 30],
        name: 's_spec_miss_2', index: ['a', 'b', 'c', 'd']);
    sSpecMiss2.setParent(dfSpecificMissing, 's_spec_miss_2');

    final defaultMissingRep =
        null; // For standalone series or df with replaceMissingValueWith = null
    ...
    group('Operator +', () {
      ...
      test('with missing values (null)', () {
        ...
        expect(
            result.data, equals([2, defaultMissingRep, 6, defaultMissingRep]));
        expect(result.index, equals(['a', 'b', 'c', 'd']));
      });
      ...
      test('with specific missing values', () {
        ...
        var result =
            sSpecMiss1 + sSpecMiss2; // Both belong to df_specific_missing
        expect(
            result.data,
            equals([
              specificMissingRep,
    ...
    group('Operator -', () {
      ...
      test('with missing values (null)', () {
        ...
        expect(
            result.data, equals([0, defaultMissingRep, 0, defaultMissingRep]));
      });
    });
    ...
    group('Operator *', () {
      ...
      test('with specific missing values', () {
        ...
        // s_spec_miss_2: [10, -999, -999, 30]
        var sB =
            Series([10, 2, 3, 30], name: 's_b', index: ['a', 'b', 'c', 'd']);
        ...
        expect(result1.data,
            equals([specificMissingRep, 1 * 2, 2 * 3, specificMissingRep]));
      });
    });
    ...
    group('Operator /', () {
      test('identical indexes', () {
        var result =
            Series([10, 20, 30], name: 'num') / Series([2, 5, 0], name: 'den');
        expect(result.data,
            equals([5.0, 4.0, defaultMissingRep])); // Division by zero
        expect(result.index, equals([0, 1, 2]));
    ...
    group('Operator ~/', () {
      test('identical indexes', () {
        var result =
            Series([10, 21, 30], name: 'num') ~/ Series([3, 5, 0], name: 'den');
        expect(
            result.data, equals([3, 4, defaultMissingRep])); // Division by zero
        expect(result.index, equals([0, 1, 2]));
      });
    });
    ...
    group('Operator %', () {
      test('identical indexes', () {
        var result =
            Series([10, 21, 30], name: 'num') % Series([3, 5, 0], name: 'den');
        expect(
            result.data, equals([1, 1, defaultMissingRep])); // Modulo by zero
      });
    });
    ...
    var sBit1 =
        Series([1, 2, 3], name: 's_bit1', index: ['a', 'b', 'c']); // 01, 10, 11
    var sBit2 =
        Series([3, 1, 0], name: 's_bit2', index: ['a', 'b', 'c']); // 11, 01, 00
    ...
    group('Operator ^ (XOR)', () {
      ...
      test('different indexes', () {
        ...
        expect(
            result.data, equals([defaultMissingRep, defaultMissingRep, 3 ^ 2]));
