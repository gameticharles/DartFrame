...
void main() {
  group('DataFrame.pivotTable', () {
    ...
    test('pivotTable with count aggregation', () {
      ...
      final pivoted = df.pivotTable(index: 'A', columns: 'B', values: 'C', aggFunc: 'count');
    ...
    test('pivotTable with min aggregation', () {
      ...
      final pivoted = df.pivotTable(index: 'A', columns: 'B', values: 'C', aggFunc: 'min');
      expect(pivoted.rows[0][1], equals(2));
      expect(pivoted.rows[0][2], equals(3));
    });
    ...
    test('pivotTable with max aggregation', () {
      ...
      final pivoted = df.pivotTable(index: 'A', columns: 'B', values: 'C', aggFunc: 'max');
    ...
    test('pivotTable with fill_value', () {
      ...
      final pivoted = df.pivotTable(index: 'A', columns: 'B', values: 'C', fillValue: 0);
      // Expected:
      // A | one | two
      //---|-----|----
      //foo| 1   | 0
     ...
     test('pivotTable with non-numeric values and min/max aggFunc', () {
      ...
      final pivotedMin = df.pivotTable(index: 'A', columns: 'B', values: 'C', aggFunc: 'min');
      ...
      final pivotedMax = df.pivotTable(index: 'A', columns: 'B', values: 'C', aggFunc: 'max');
  ...
  group('DataFrame.pivot (strict)', () {
    ...
    test('Strict pivot with implicit value column (one remaining)', () {
      ...
      final pivoted = df.pivot(index: 'index_col', columns: 'column_col'); // values='value_col' inferred
     ...
     test('Strict pivot with implicit value column (multiple remaining - uses first)', () {
      final df = DataFrame.fromRows([
  ...
  group('DataFrame.crosstab', () {
    ...
    test('Basic crosstab (counts)', () {
      ...
      expect(ct.row({'A': 'baz'})['two'], equals(0)); // Assuming missing combinations are 0
    });

    test('Crosstab with values and aggFunc=sum', () {
        final ct = df.crosstab(index: 'A', column: 'B', values: 'C', aggfunc: 'sum');
        // B      one  two
        // A
        // foo     30   30   (10+20)
        // bar     40  110   (50+60)
        ...
        expect(ct.row({'A': 'baz'})['two'], equals(0)); // No 'baz' 'two' combo, sum is 0
    });
    ...
    test('Crosstab with normalize=true (all)', () {
        ...
        expect(ct.row({'A': 'foo'})['two'], closeTo(1 / totalObservations, 0.001));
        // ... check other cells
    });
    ...
    test('Crosstab with normalize=\'all\' and margins=true', () {
      ...
      // Check a few values
      expect(ct.row({'A': 'foo'})['one'], closeTo(2 / totalObservations, 0.001));
      expect(ct.row({'A': 'foo'})['Overall'], closeTo(3 / totalObservations, 0.001));
      
      final totalRow = ct.rows.firstWhere((r) => r[0] == 'Overall');
      expect(totalRow[ct.columns.indexOf('one')], closeTo(4 / totalObservations, 0.001));
      expect(totalRow[ct.columns.indexOf('Overall')], closeTo(1.0, 0.001)); // Grand total of proportions
    });
    ...
    test('Crosstab with values, aggFunc, normalize, and margins', () {
      ...
      // Check 'foo' row
      expect(ct.row({'A':'foo'})['one'], closeTo(30.0/60.0, 0.001)); // (10+20)/60
      expect(ct.row({'A':'foo'})['two'], closeTo(30.0/60.0, 0.001)); // 30/60
      expect(ct.row({'A':'foo'})['All'], closeTo(1.0, 0.001)); // (30+30)/60 = 1.0
      ...
      expect(totalRow[ct.columns.indexOf('one')], closeTo(expectedAllOne, 0.001));
      expect(totalRow[ct.columns.indexOf('two')], closeTo(expectedAllTwo, 0.001));
      expect(totalRow[ct.columns.indexOf('All')], closeTo(expectedAllAll, 0.001));
    });
  ...
  group('DataFrame.bin', () {
    ...
    test('bin with int bins, default right=true, include_lowest=false', () {
      ...
      expect(binned.column('value_bin').data, equals([
    ...
    test('bin with int bins, right=true, include_lowest=true', () {
      final binned = df.bin('value', 3, newColumn: 'value_bin', includeLowest: true);
      // Edges: 1, 4, 7, 10
      // Bins: [1, 4], (4, 7], (7, 10]
      expect(binned.column('value_bin').data, equals([
        '[1.00, 4.00]', // 1
    ...
    test('bin with int bins, right=false, include_lowest=false (effectively true for first bin)', () {
      ...
      expect(binned.column('value_bin').data, equals([
        ...
        '[7.00, 10.00]', '[7.00, 10.00]', '[7.00, 10.00]', '[7.00, 10.00]', // 7,8,9,10
      ]));
    });
    ...
    test('bin with List<num> bins', () {
      ...
      expect(binned.column('value_bin_list').data, equals([
        '(0.00, 5.00]', '(0.00, 5.00]', '(0.00, 5.00]', '(0.00, 5.00]', '(0.00, 5.00]', // 1,2,3,4,5
        '(5.00, 10.00]', '(5.00, 10.00]', '(5.00, 10.00]', '(5.00, 10.00]', '(5.00, 10.00]', // 6,7,8,9,10
      ]));
    });

    test('bin with List<num> bins and include_lowest=true', () {
      final binned = df.bin('value', [1, 5, 10], newColumn: 'value_bin_list_incl', includeLowest: true);
      // Bins: [1,5], (5,10]
      expect(binned.column('value_bin_list_incl').data, equals([
        '[1.00, 5.00]', '[1.00, 5.00]', '[1.00, 5.00]', '[1.00, 5.00]', '[1.00, 5.00]', // 1,2,3,4,5
        '(5.00, 10.00]', '(5.00, 10.00]', '(5.00, 10.00]', '(5.00, 10.00]', '(5.00, 10.00]', // 6,7,8,9,10
      ]));
    });
    
    test('bin with List<num> bins, right=false', () {
      final binned = df.bin('value', [1, 5, 10], newColumn: 'value_bin_list_rf', right:false);
      // Bins: [1,5), [5,10]
      expect(binned.column('value_bin_list_rf').data, equals([
        '[1.00, 5.00)', '[1.00, 5.00)', '[1.00, 5.00)', '[1.00, 5.00)', // 1,2,3,4
        '[5.00, 10.00]', '[5.00, 10.00]', '[5.00, 10.00]', '[5.00, 10.00]', '[5.00, 10.00]', '[5.00, 10.00]', // 5,6,7,8,9,10
      ]));
    });
    ...
    test('bin with duplicate bin edges and duplicates="drop"', () {
      final binned = df.bin('value', [1, 5, 5, 10], newColumn: 'value_bin_dup_drop', duplicates: 'drop');
      ...
      // Values: 1 (null), 2,3,4,5 (bin1) | 6,7,8,9,10 (bin2)
       expect(binned.column('value_bin_dup_drop').data, equals([
        null, // 1 is not > 1
        '(1.00, 5.00]', '(1.00, 5.00]', '(1.00, 5.00]', '(1.00, 5.00]', // 2,3,4,5
        '(5.00, 10.00]', '(5.00, 10.00]', '(5.00, 10.00]', '(5.00, 10.00]', '(5.00, 10.00]', // 6,7,8,9,10
      ]));
    });

    test('bin with provided labels', () {
      final binned = df.bin('value', 2, newColumn: 'value_bin_labels', labels: ['low', 'high']);
      // Edges: 1, 5.5, 10. Bins: (1, 5.5], (5.5, 10]
      // 1 (null), 2,3,4,5 (low) | 6,7,8,9,10 (high)
      expect(binned.column('value_bin_labels').data, equals([
        null, 'low', 'low', 'low', 'low',
    ...
    test('bin with single unique value in column', () {
      ...
      expect(binned.column('value_bin_single').data.every((e) => e != null), isTrue);
      expect(binned.column('value_bin_single').data.length, equals(3));
    });

    test('bin on empty data throws error', () {
      final dfEmpty = DataFrame.fromRows([{'value': double.nan}]); // Effectively empty numericData
