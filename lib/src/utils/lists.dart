...
class ListEqualConfig {
  ...
  ListEqualConfig copyWith({
    ...
    return ListEqualConfig(
      ...
      enableCircularReferenceDetection: enableCircularReferenceDetection ?? this.enableCircularReferenceDetection,
      optimizeForLargeLists: optimizeForLargeLists ?? this.optimizeForLargeLists,
      enableParallelProcessing: enableParallelProcessing ?? this.enableParallelProcessing,
      customComparableTypes: customComparableTypes ?? this.customComparableTypes,
...
class ListComparisonException implements Exception {
  ...
  const ListComparisonException(this.message, [this.context, this.originalError]);
...
/// `false` otherwise.
bool listEqual(List<List<dynamic>> lists, [ListEqualConfig config = const ListEqualConfig()]) {
...
ListComparisonResult listEqualDetailed(List<List<dynamic>> lists, [ListEqualConfig config = const ListEqualConfig()]) {
  final stopwatch = Stopwatch()..start();
  
  try {
    // Input validation
    if (lists.length < 2) {
      throw const ListComparisonException('At least 2 lists are required for comparison');
    }
    ...
    if (config.mode != ListComparisonMode.unordered || !config.optimizeForLargeLists) {
      ...
      if (!lists.every((list) => list.length == firstLength)) {
        ...
        return ListComparisonResult(
          ...
          differences: ['Length mismatch: ${lists.map((l) => l.length).toList()}'],
          elapsedTime: stopwatch.elapsed,
        );
      }
    }
    ...
    throw ListComparisonException('Unexpected error during comparison', null, e);
  }
}

/// Optimized comparison for large lists
ListComparisonResult _optimizedComparison(List<List<dynamic>> lists, ListEqualConfig config, _ComparisonContext context) {
...
bool _sampleVerification(List<List<dynamic>> lists, ListEqualConfig config, _ComparisonContext context) {
...
bool _compareTwoLists(List<dynamic> list1, List<dynamic> list2, ListEqualConfig config, _ComparisonContext context) {
  // Handle empty lists
  ...
  if (context.currentDepth >= config.maxDepth) {
    throw ListComparisonException('Maximum recursion depth exceeded', 'Depth: ${context.currentDepth}');
  }
...
/// Strict comparison: order matters, types must match exactly
bool _strictComparison(List<dynamic> list1, List<dynamic> list2, ListEqualConfig config, _ComparisonContext context) {
  if (list1.length != list2.length) {
    context.addDifference('Length mismatch: ${list1.length} vs ${list2.length}');
    return false;
  }
  
  for (int i = 0; i < list1.length; i++) {
    if (!_compareElements(list1[i], list2[i], config, context)) {
      context.addDifference('Elements differ at index $i: ${list1[i]} vs ${list2[i]}');
      return false;
    }
  }
  return true;
...
bool _unorderedComparison(List<dynamic> list1, List<dynamic> list2, ListEqualConfig config, _ComparisonContext context) {
  if (list1.length != list2.length) {
    context.addDifference('Length mismatch: ${list1.length} vs ${list2.length}');
    return false;
  }
...
/// Deep comparison: recursively compares nested structures
bool _deepComparison(List<dynamic> list1, List<dynamic> list2, ListEqualConfig config, _ComparisonContext context) {
  if (list1.length != list2.length) {
    context.addDifference('Length mismatch: ${list1.length} vs ${list2.length}');
    return false;
  }
...
bool _flexibleComparison(List<dynamic> list1, List<dynamic> list2, ListEqualConfig config, _ComparisonContext context) {
  if (list1.length != list2.length) {
    context.addDifference('Length mismatch: ${list1.length} vs ${list2.length}');
    return false;
  }
...
bool _compareElements(dynamic elem1, dynamic elem2, ListEqualConfig config, _ComparisonContext context) {
...
bool _deepCompareElements(dynamic elem1, dynamic elem2, ListEqualConfig config, _ComparisonContext context) {
  if (elem1 == null && elem2 == null) return true;
  if (elem1 == null || elem2 == null) return false;
  ...
  if (elem1 is List || elem1 is Map || elem1 is Set) {
    if (!context.addVisitedObject(elem1)) {
      throw const ListComparisonException('Circular reference detected in first element');
    }
  }
  
  if (elem2 is List || elem2 is Map || elem2 is Set) {
    if (!context.addVisitedObject(elem2)) {
      throw const ListComparisonException('Circular reference detected in second element');
    }
  }
...
bool _flexibleCompareElements(dynamic elem1, dynamic elem2, ListEqualConfig config, _ComparisonContext context) {
  if (elem1 == null && elem2 == null) return true;
  if (elem1 == null || elem2 == null) return false;
  ...
  if (config.allowTypeCoercion) {
    ...
    if (_compareStrings(elem1.toString(), elem2.toString(), config)) return true;
...
bool _compareMaps(Map map1, Map map2, ListEqualConfig config, _ComparisonContext context) {
  if (map1.length != map2.length) return false;
  
  for (final key in map1.keys) {
    if (!map2.containsKey(key)) return false;
    if (!_deepCompareElements(map1[key], map2[key], config, context)) return false;
  }
...
/// Compare two sets
bool _compareSets(Set set1, Set set2, ListEqualConfig config, _ComparisonContext context) {
...
extension ListEqualityExtension<T> on List<T> {
  ...
  bool isEqualTo(List<T> other, [ListEqualConfig config = const ListEqualConfig()]) {
    return listEqual([this, other], config);
  }
  
  /// Get detailed comparison result with another list
  ListComparisonResult compareDetailedWith(List<T> other, [ListEqualConfig config = const ListEqualConfig()]) {
...
void main() {
  ...
  print('\n=== Unordered Comparison ===');
  print(listEqual([[1, 2, 3], [3, 2, 1]], ListEqualPresets.unordered)); // true
  print(listEqual([['a', 'b', 'c'], ['c', 'a', 'b']], ListEqualPresets.unordered)); // true
  
  // String options
  print('\n=== String Comparison Options ===');
  print(listEqual([['Hello', ' World '], ['hello', 'world']], ListEqualPresets.flexibleString)); // true
  ...
  // Flexible comparison
  print('\n=== Flexible Comparison ===');
  print(listEqual([['1', '2'], [1, 2]], ListEqualPresets.flexible)); // true
  ...
  print(listEqual([[date1], [date2]], customConfig)); // true
  
  // Detailed comparison results
  print('\n=== Detailed Comparison Results ===');
  final result = listEqualDetailed([[1, 2, 3], [1, 2, 4]]);
  print('Result: $result');
  ...
  final perfResult = listEqualDetailed([largeList1, largeList2], ListEqualPresets.optimized);
  print('Large list comparison: ${perfResult.isEqual} (${perfResult.elapsedTime.inMicroseconds}μs)');
  ...
  try {
    ...
    final result = listEqualDetailed([circularList, [1, 2, 'something']], ListEqualPresets.safeDeep);
  ...
  print('Complex nested: ${listEqual([complex1, complex2], ListEqualPresets.deep)}'); // true
  
  // Mixed type flexible comparison
  print('\n=== Mixed Type Flexible Comparison ===');
  final mixed1 = ['1', 2.0, true, '4.5'];
  final mixed2 = [1, 2, 1, 4.5];
  print('Mixed types flexible: ${listEqual([mixed1, mixed2], ListEqualPresets.flexible)}'); // true
