...
extension GeoSeriesGeoProcess on GeoSeries {
  ...
  GeoSeries buffer({
    ...
    if (distance is num) {
      ...
      if (distance.length != data.length) {
        throw ArgumentError('Distance list must have the same length as the GeoSeries');
      }
      distances = distance.map((d) => d is num ? d.toDouble() : 0.0).toList();
    } else if (distance is Series) {
      // Series of distances
      if (distance.length != data.length) {
        throw ArgumentError('Distance series must have the same length as the GeoSeries');
      }
      distances = distance.data.map((d) => d is num ? d.toDouble() : 0.0).toList();
    } else {
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      if (geom is GeoJSONGeometry) {
        bufferedGeometries.add(_bufferGeometry(
          geom, 
          dist, 
          resolution, 
          capStyle, 
  ...
  GeoJSONGeometry _bufferGeometry(
    ...
    if (geometry is GeoJSONPoint) {
      ...
      return _bufferLineString(
        geometry, distance, resolution, capStyle, joinStyle, mitreLimit, singleSided);
      ...
      return _bufferMultiPoint(
        geometry, distance, resolution);
    } else if (geometry is GeoJSONMultiLineString) {
      return _bufferMultiLineString(
        geometry, distance, resolution, capStyle, joinStyle, mitreLimit, singleSided);
  ...
  GeoJSONPolygon _bufferPoint(GeoJSONPoint point, double distance, int resolution) {
    if (distance <= 0) {
      return GeoJSONPolygon([[[0, 0], [0, 0], [0, 0], [0, 0]]]); // Empty polygon
    }
  ...
  GeoJSONGeometry _bufferLineString(
    ...
    if (capStyle == 'round') {
      ...
      _addRoundCap(coords.first, coords[1], distance, resolution, true, leftSide, rightSide);
      _addRoundCap(coords.last, coords[coords.length - 2], distance, resolution, false, leftSide, rightSide);
    } else if (capStyle == 'square') {
      // Add square caps
      _addSquareCap(coords.first, coords[1], distance, leftSide, rightSide);
      _addSquareCap(coords.last, coords[coords.length - 2], distance, leftSide, rightSide);
    }
  ...
  void _addSquareCap(
    ...
    if (length > 0) {
      ...
      final leftCorner = [endPoint[0] + perpX + extX, endPoint[1] + perpY + extY];
      final rightCorner = [endPoint[0] - perpX + extX, endPoint[1] - perpY + extY];
      ...
      if (leftSide.isEmpty || rightSide.isEmpty) {
        ...
        if (leftSide.first[0] == endPoint[0] + perpX && leftSide.first[1] == endPoint[1] + perpY) {
  ...
  GeoJSONGeometry _bufferPolygon(
    ...
    final bufferedOuterRing = _bufferRing(outerRing, distance, resolution, joinStyle, mitreLimit, false);
  ...
  GeoJSONGeometry _bufferMultiPoint(
    ...
    for (final point in coordinates) {
      final bufferedPoint = _bufferPoint(GeoJSONPoint(point), distance, resolution);
  ...
  GeoJSONGeometry _bufferMultiPolygon(
