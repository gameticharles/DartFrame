...
class PerformanceTestRunner {
  ...
  void _generateReport() {
    ...
    List<PerformanceResult> successfulResults = _results.where((r) => r.success).toList();
    List<PerformanceResult> failedResults = _results.where((r) => !r.success).toList();
    
    print('Summary:');
    print('- Total tests: ${_results.length}');
    print('- Successful: ${successfulResults.length}');
    print('- Failed: ${failedResults.length}');
    print('- Success rate: ${(successfulResults.length / _results.length * 100).toStringAsFixed(1)}%\n');
  ...
  void _printPerformanceAnalysis(List<PerformanceResult> results) {
    ...
    double avgTime = executionTimes.reduce((a, b) => a + b) / executionTimes.length;
    ...
    if (memoryUsages.isNotEmpty) {
      memoryUsages.sort();
      double avgMemory = memoryUsages.reduce((a, b) => a + b) / memoryUsages.length;
    ...
    if (throughputs.isNotEmpty) {
      throughputs.sort();
      double avgThroughput = throughputs.reduce((a, b) => a + b) / throughputs.length;
  ...
  void _saveReportToFile() {
    try {
      ...
      for (PerformanceResult result in _results) {
        ...
        if (result.memoryUsage != null) {
          buffer.writeln('  Memory usage: ${_formatBytes(result.memoryUsage!)}');
        }
        
        if (result.throughput != null) {
          buffer.writeln('  Throughput: ${result.throughput!.toStringAsFixed(2)} ops/sec');
        }
...
class VectorizedOperationsTest extends PerformanceTest {
  ...
  Future<PerformanceResult> run() async {
    ...
    double throughput = (dataSize * 3) / (stopwatch.elapsedMicroseconds / 1000000); // 3 operations
...
class MemoryOptimizationTest extends PerformanceTest {
  ...
  Future<PerformanceResult> run() async {
    ...
    final data = {
      'integers_as_doubles': List.generate(dataSize, (i) => (i % 1000).toDouble()),
      'small_integers': List.generate(dataSize, (i) => i % 128),
      'large_integers': List.generate(dataSize, (i) => i * 10000),
      'actual_doubles': List.generate(dataSize, (i) => Random().nextDouble() * 100),
    };
...
class CachePerformanceTest extends PerformanceTest {
  ...
  Future<PerformanceResult> run() async {
    ...
    double throughput = operationCount / (stopwatch.elapsedMicroseconds / 1000000);
